...
2. When running on JRE 21 use virtual threads to prepare participants when asynchronous prepare is configured (CoordinatEnvironmentBean#asyncPrepare) and use a virtual thread to commit and rollback transactions when asynchronous commit and abort are configured (CoordinatEnvironmentBean#asyncCommit). Include a configuration property to switch back to using the old thread pool - the one that reuses a fixed number of threads operating off a shared unbounded queue (see java.util.Executors#newFixedThreadPool). The property name will be CoordinatorEnvironmentBean#useVirtualThreadsForTwoPhaseCommitThreads.
...
Note, however, that when running on JRE 21 a config option is provided for reverting to the use of the old fixed size thread pool.
...
JEP 444 promises performance improvements in scalability, throughput and thread utilisation so a benchmark to show the effect of using virtual threads will be provided.
